...
const OrderItem = ({ name, imageUrl, size, quantity, totalPrice }) => {
    return (
        <div className="grid sm:grid-cols-2 items-start gap-6">
            <div className="max-w-[190px] px-4 ">
            ...
            <div>
                ...
                <ul className="text-xs text-gray-700 space-y-2 mt-2">
                    <li className="flex flex-wrap gap-4">Size <span className="ml-auto">{size}</span></li>
                    <li className="flex flex-wrap gap-4">Quantity <span className="ml-auto">{quantity}</span></li>
                    <li className="flex flex-wrap gap-4">Total Price <span className="ml-auto">${parseInt(totalPrice) * quantity}</span></li>
                </ul>
            </div>
        </div>
    );
    ...
    const getProductBasedOnId = influencers.flatMap(influencer =>
        influencer.categories.flatMap(category =>
            category.subcategory.flatMap(subcategory =>
                subcategory.products.filter(product => product.id === parseInt(props.searchParams.id))
            )
        )
    );
    const orderItems = [
        {
            imageUrl: `/${getProductBasedOnId[0].img}`,
            name: getProductBasedOnId[0].name,
            size: "M",
            totalPrice: `${getProductBasedOnId[0].price} L`,
            quantity: props.searchParams.quantity
        },
    ];
                    ...
                    <div className="lg:col-span-2 max-lg:order-1 p-6 max-w-4xl mx-auto w-full">
                        ...
                        <form className="lg:mt-12">
                            ...
                            <div className="mt-12">
                                ...
                                <div className="grid gap-6 mt-8">
                                    <input type="text" placeholder="Cardholder's Name"
                                           className="px-2 py-3.5 bg-white text-[#333] w-full text-sm border-b-2 focus:border-[#333] outline-none"/>
                                    <div
                                        className="flex bg-white border-b-2 focus-within:border-[#333] overflow-hidden">
                            ...
                            <div className="flex flex-wrap gap-4 mt-8">
                                <button type="button"
                                        className="min-w-[150px] px-6 py-3.5 text-sm bg-gray-100 text-[#333] rounded-md hover:bg-gray-200">Back
                                </button>
                                <button type="button"
                                        className="min-w-[150px] px-6 py-3.5 text-sm bg-[#333] text-white rounded-md hover:bg-[#111]">Confirm
                                    payment $240
                                </button>
                            </div>
                        </form>
                    ...
                    <div className="bg-gray-100 lg:h-screen lg:sticky lg:top-0">
                        <div className="relative h-full">
                            <div className="p-8 lg:overflow-auto lg:h-[calc(100vh-60px)] max-lg:mb-8">
                                <h2 className="text-2xl font-extrabold text-gray-700">Order Summary</h2>
                            ...
                            <div className="absolute left-0 bottom-0 bg-gray-200 w-full p-4">
                                <h4 className="flex flex-wrap gap-4 text-base text-gray-700 font-bold">Total <span
                                    className="ml-auto">$240.00</span></h4>
                            </div>
                        </div>
                    </div>
                </div>
